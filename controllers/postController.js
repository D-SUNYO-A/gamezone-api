  ...
  updatePost = async (req, res, id) => {
    try {
      ...
      if (!post) {
        ...
        // Utilisez le middleware d'upload pour gérer les fichiers
        uploadMiddleware.upload(req, res, async (err) => {
          if (err) {
            console.error(err);
            responseUtil.sendResponse(res, 500, 'Internal Server Error');
          } else {
            ...
            // Mise à jour des images si disponibles
            if (req.files['images']) {
              post.images = req.files['images'].map((file) => file.path);
            }
  
            // Mise à jour des vidéos si disponibles
            if (req.files['videos']) {
